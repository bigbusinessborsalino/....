...
def load_users():
    ...
    if pm_users_col is not None:
        ...
        logger.warning("No MONGO_URI – using empty set (users lost on restart)")
        users = set()
...
def add_user(user_id):
    ...
    if pm_users_col is not None:
        ...
        logger.warning("No DB connection – user stored only in memory")
...
async def check_force_sub(user_id):
    ...
    for ch in FORCE_SUB_CHANNELS:
        try:
            ...
            logger.error(f"Force sub check failed for {ch}: {e}")
            # If we cannot verify, assume not subscribed (safer)
            return False
    return True
...
async def anime_download(client, message: Message):
    ...
    if not await check_force_sub(message.from_user.id):
        buttons = []
        for ch in FORCE_SUB_CHANNELS:
            clean = ch.replace("-100", "") if ch.startswith("-") else ch
    ...
    # Then, check if user has started the bot in PM
